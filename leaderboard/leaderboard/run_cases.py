...
from SBT.scenario_parser import ego_vehicle_parser, other_vehicle_parser, weather_parser, lane_parser, fill_junction
...
class TestCase(object):
    ...
    def __init__(self, args, statistics_manager):
        self.init(args, statistics_manager)

    def init(self, args, statistics_manager):
        """
    ...
    def _cleanup(self):
        ...
        """
        # print('_cleanup, called by:', traceback.extract_stack[-2][2])
    ...
    def _load_and_wait_for_world(self, town, weather):
        """
        ...
        CarlaDataProvider.set_traffic_manager_port(int(self.args.trafficManagerPort))
        CarlaDataProvider.set_weather(weather)

        self.traffic_manager.set_synchronous_mode(True)
        self.traffic_manager.set_random_device_seed(int(self.args.trafficManagerSeed))
        ...
        print('Map:', CarlaDataProvider.get_map().name)
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            print('call _cleanup in:')
            print('    _load_and_run_scenario')
            print('    init scenario, exeption 1')
            self._cleanup()
            sys.exit(-1)
        except Exception as e:
            ...
            print('call _cleanup in:')
            print('    _load_and_run_scenario')
            print('    init scenario, exeption 2')
            self._cleanup()
            return
        ...
        try:
            ...
            lane_vector = lane_parser(config.distance_vec, config.vec_len)

            scenario = None

            current_waypoint = current_map.get_waypoint(start_location)

            waypoint_other_vehicle = fill_junction(current_waypoint, lane_vector) #region
            numb_other_vehicle = len(waypoint_other_vehicle)-1

            config.trajectory[0] = waypoint_other_vehicle[0].transform.location
            waypoint_other_vehicle = waypoint_other_vehicle[1:]
            ...
            print('call _cleanup in:')
            print('    _load_and_run_scenario')
            print('    try to run scenario, exeption 1')
            self._cleanup()
            sys.exit(-1)
        ...
        # traffic_manager.global_percentage_speed_difference(70) # SPEED. base speed 30 km/h, increased by 50%
    ...
    def run_one_case(self, scenario_vec, config):
        ...
        """
        case_start_time = GameTime.get_time()

        print('Case', self.case_count)

        # 9 weather, 2 position offset
        config.repetition_index = 0

        config.weather_vec = scenario_vec[0:9]
        config.ego_vehicle_vec = scenario_vec[9:9+2] #update should be later, as we donot have map in it
        # config.other_vehicle_vec= scenario_vec[9+2:] 
        
        config.distance_vec = scenario_vec[9+2:]
        config.vec_len = 15
        ...
        case_end_time = GameTime.get_time()
    ...
    def run_cases(self, scenario_vecs, config):
        """
        Run the challenge mode
        """
        for scenario_vec in scenario_vecs:
            print("Run Scenario")
            print("Town:", config.town)
            self._load_and_wait_for_world(config.town, config.weather)
            self.run_one_case(scenario_vec, config)
            # print('call _cleanup in:')
            # print('    run_cases')
            self._cleanup()
            if hasattr(self, 'client') and self.statistics_manager:
                self.statistics_manager.scenario = None
                gc.collect()
        
        self._end_time = GameTime.get_time()
        print(f"Total Time: {self._start_time-self._end_time:.2f} seconds\n\n")
