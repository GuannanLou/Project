...
class InRouteTest(Criterion):
    ...
    def __init__(self, actor, route, offroad_min=-1, offroad_max=30, name="InRouteTest", terminate_on_failure=False):
        ...
        # print([actor.type_id for actor in all_actors])
        self._vechicles = []
        self._meshs = []
        self._pedestrians = []
        for actor in all_actors:
            if 'vehicle' in actor.type_id:
                self._vechicles.append(actor)

        self._debug = 0

        # Get distance from center lane
        # Get min distance from other vehicle
    ...
    def update(self):
        ...
        # Get min distance from static mesh
        # Get distance from destination
        current_fitness_score = [0,(1000,0,0,0),1000,1000,1000]
    ...
    def follow_the_center_of_the_lane(self, shortest_distance):
        shortest_distance = 1-min([shortest_distance,1.15])/1.15
        if self._debug==1:
            color = ['↔️  ','']
    ...
    def bounding_box_distance(self, actor1, actor2):
        bbox_1 = actor1.bounding_box.get_world_vertices(actor1.get_transform())
        bbox_2 = actor2.bounding_box.get_world_vertices(actor2.get_transform())

        distances = []
        for location_1 in bbox_1:
            location_1.z = 0
            for location_2 in bbox_2:
                location_2.z = 0
    ...
    def get_min_distance_from_other_vehicle(self,ego_vehicle, world): # include bicycle
        distances = [(1000,0)]
        ego_vehicle_location = (ego_vehicle.get_location().x, ego_vehicle.get_location().y)
        ...
        if self._debug == 1:
            color = ['↔️  ','']
            if shortest_distance > self._last_fitness_score[1]:
                color = ['⬆️  \033[92m','\033[0m']
            elif shortest_distance < self._last_fitness_score[1]:
                color = ['⬇️  \033[91m','\033[0m']
            print("Minimum Distance from other Vehicle: " + color[0] + str(shortest_distance) + color[1])
        return (shortest_distance[0], shortest_distance[1], ego_vehicle_location[0], ego_vehicle_location[1]) # substracting distances from center of vehicle
    ...
    def get_min_distance_from_pedestrians(self, ego_vehicle, world):
        ...
        # shortest_distance = (min(distances)) - 1.2
        shortest_distance = (min(distances))
        if self._debug == 1:
            color = ['↔️  ','']
            if shortest_distance > self._last_fitness_score[2]:
                color = ['⬆️  \033[92m','\033[0m']
            elif shortest_distance < self._last_fitness_score[2]:
                color = ['⬇️  \033[91m','\033[0m']
    ...
    def get_min_distance_from_static_mesh(self, ego_vehicle, world):
        ...
        if self._debug == 1:
            color = ['↔️  ','']
            if shortest_distance > self._last_fitness_score[3]:
                color = ['⬆️  \033[92m','\033[0m']
            elif shortest_distance < self._last_fitness_score[3]:
                color = ['⬇️  \033[91m','\033[0m']
    ...
    def destney_completeness(self, ego_vehicle, world):
        ...
        closest_i, cloest_dist = 0, ego_vehicle_location.distance(self._waypoints[0])
        for i, waypoint in enumerate(self._waypoints):
            dist = ego_vehicle_location.distance(waypoint)
            if dist < cloest_dist:
                closest_i, cloest_dist = i, dist

        shortest_distance = (closest_i+1)/len(self._waypoints)
        if self._debug == 1:
            color = ['↔️  ','']
            if shortest_distance > self._last_fitness_score[4]:
                color = ['⬆️  \033[92m','\033[0m']
...
class RunningStopTest(Criterion):
    ...
    def update(self):
        ...
        if not self._target_stop_sign:
            ...
            # we were in the middle of dealing with a stop signcollision
